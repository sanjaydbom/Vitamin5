...
static void start_process(void *file_name_) {
    ...
    if(success) {
        ...
        int align = (uintptr_t)if_.esp % 4;
        if (align != 0) {
            if_.esp -= align;
            memset(if_.esp, 0, align);
        }
        int arg_data_size = (argc + 4) * 4;

        uintptr_t final_esp = (uintptr_t)if_.esp - arg_data_size;

        int padding_16 = (final_esp - 12 + 16) % 16;
            
        if (padding_16 != 0) {
            if_.esp -= padding_16;
            memset(if_.esp, 0, padding_16);
        }
